    ...
    pub fn create(
        ...
        match lookup_info.lookup_used(gates) {
            ...
            Some(lookup_used) => {
                ...
                let (runtime_table_offset, runtime_selector) = if let Some(runtime_tables) =
                    &runtime_tables
                {
                    ...
                    let runtime_selector = {
                        ...
                        evals.extend(iter::repeat(F::zero()).take(runtime_table_offset));
                        evals.extend(iter::repeat(F::one()).take(d1_size - runtime_table_offset));
